...
{
  if (_usb_ehci_get_interrupt_status(USB_PORT_CHANGE_INTERRUPT))
  { // Handle port change
    _usbd_enumerate_devices();
    _usb_ehci_clear_interrupt(USB_PORT_CHANGE_INTERRUPT); // Clear the port change bit
  }
}
...
void _usbd_set_device_address(USBDev *dev, uint8 newAddr)
{
  ...
  USBPacket setupPacket = _usbd_new_packet(USB_SETUP_PACKET, 8); // New setup packet
  ...
  USBPacket statusPacket = _usbd_new_packet(USB_IN_PACKET, 0); // The status packet
...
void _usbd_get_device_desc(USBDev *dev, void *dst, uint8 descType, uint8 descIndex)
{
  ...
  // The size of a descriptor is at most 64 bytes. Get a packet that can handle that much
...
void _usbd_get_string_desc(USBDev *dev, void *dst, uint8 descIndex)
{
  ...
  usb_device_request_packet(data, 0x80, GET_DESCRIPTOR, usb_bytes_to_word(STRING, descIndex),
                            0x0409, 64); // Get a string descriptor. 0x0409 is language code for
                                         // english.
  _usbd_add_data_packet(setupPacket, data, 8);
...
void _usbd_enumerate_devices(void)
{
  ...
  if (_usb_ehci_has_port_change())
  {
    ...
    while (_usb_ehci_find_port_change() != 0)
    {
      ...
      for (int i = 0; i < recBuff[4]; ++i)
      { // Loop through all interface descriptors

        uint8 *endpDesc = (intDesc + intDesc[0]);
        for (int e = 0; e < intDesc[4]; ++e)
...
/**
** Create and initialize and endpoint
 */
